...
联系邮箱已设置为 `z790977870@gmail.com`。如需变更，请同步修改 `index.html` 中的邮箱文字和 `mailto:` 链接。核对页面描述与应用实际行为一致；页面本身不会实现数据访问限制或删除功能。后续修改政策时更新日期。
...
- 包含官网标识的隐私政策链接：https://z790977870-bot.github.io/id-design-reference-archive/bszhdesign/privacy.html
...
修改 `index.html` 后，将同样内容同步到 `bszhdesign/privacy.html` 和 `深圳市博思纵横设计有限公司/privacy.html`，再提交到 `main`。三个地址提供相同政策。配置好 Pages 后会自动重新发布。此页面用于披露应用的数据处理方式，不代表 Pinterest 已批准应用或其数据使用方式。
